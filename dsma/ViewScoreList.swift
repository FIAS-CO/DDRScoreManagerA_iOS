    ...
    internal func refreshAllInternal() {
        ...
        OperationQueue().addOperation({ () -> Void in
            ...
            if(self.rparam_ParentCategory == "Display All Pattern") {
                ...
                switch self.rparam_ParentCategory {
                case "Series Title":
                    if self.rparam_Category == "WORLD" { self.mTitle = "DDR WORLD" }
                    ...
                    if self.rparam_Category != "WORLD" { musicFilter.SerWORLD = false }
            ...
            case NSLocalizedString("from GATE (simple)", comment: "ViewScoreList"):
                action = { ()->Void in
                    ...
                    if (preferences.Gate_LoadFrom == .world) {
                        DialogUtil.showDataFetchOptions(from: self, processPool: self.rparam_ProcessPool, refreshAfter: true)
                    } else {
